...
class employeeClass:
    def __init__(self, root):
        ...
        cmb_search = ttk.Combobox(SearchFrame, textvariable=self.var_searchby, values=(
            "Select", "Email", "Name", "Contact"), state='readonly', justify=CENTER)
        cmb_search.place(x=10, y=10, width=180)
        cmb_search.current(0)

        txt_search = Entry(SearchFrame, textvariabl=self.var_searchtxt, font=(
            "goudy old style", 12), bg="lightyellow").place(x=200, y=10)
        btn_search = Button(SearchFrame, text="search", command=self.search, font=(
            "goudy old style", 15), bg="#4caf50", fg="white", cursor="hand2").place(x=380, y=9, width=150, height=26)

    # Title-------------

        title = Label(self.root, text="Employee Details", bg="blue",
                      fg="white").place(x=50, y=100, width=1000)

    # content--------------
    # row 1----------------------
        lbl_empid = Label(self.root, text="Emp ID", font=(
            "goudy old style", 13), bg="white").place(x=50, y=150)
        lbl_gender = Label(self.root, text="Gender", font=(
            "goudy old style", 13), bg="white").place(x=400, y=150)
        lbl_contact = Label(self.root, text="Contact", font=(
            "goudy old style", 13), bg="white").place(x=750, y=150)
        ...
        cmb_gender = ttk.Combobox(self.root, textvariable=self.var_gender, values=(
            "Select", "Male", "Female", "other"), state='readonly', justify=CENTER)
        cmb_gender.place(x=500, y=153, width=180)
    ...
    # Row 2---------------
        ...
        lbl_dob = Label(self.root, text="D.O.B", font=(
            "goudy old style", 13), bg="white").place(x=400, y=190)
    ...
    # row 3----------------------
        ...
        lbl_password = Label(self.root, text="Password", font=(
            "goudy old style", 13), bg="white").place(x=400, y=230)
        ...
        cmb_utype = ttk.Combobox(
            self.root, textvariable=self.var_utype, values=("Select", "Employee", "Admin"))
    ...
    # row 4----------------------
        ...
        lbl_salary = Label(self.root, text="Salary", font=(
            "goudy old style", 13), bg="white").place(x=350, y=270, width=180)
        ...
        txt_salary = Entry(self.root, textvariable=self.var_salary, font=(
            "goudy old style", 13), bg="lightyellow").place(x=500, y=270, width=180)

    # button for the delete save update clear---------------

        btn_save = Button(self.root, text="Save", command=self.add, font=("goudy old style", 13),
                          bg="#4caf50", fg="white", cursor="hand2").place(x=420, y=310, width=110, height=28)
        btn_update = Button(self.root, text="Update", command=self.update, font=(
            "goudy old style", 13), bg="#4caf50", fg="white", cursor="hand2").place(x=560, y=310, width=110, height=28)
        btn_delete = Button(self.root, text="Delete", command=self.delete, font=(
            "goudy old style", 13), bg="#4caf50", fg="white", cursor="hand2").place(x=700, y=310, width=110, height=28)
        btn_clear = Button(self.root, text="Clear", command=self.clear, font=(
            "goudy old style", 13), bg="#4caf50", fg="white", cursor="hand2").place(x=840, y=310, width=110, height=28)
        ...
        self.EmployeeTable = ttk.Treeview(emp_frame, column=("eid", "name", "email", "gender", "contact", "dob",
    ...
    def add(self):
        ...
        try:
            if self.var_emp_id.get() == "":
                ...
                if row != None:
                    messagebox.showerror(
                        "Error", "This Employee ID already assinged, try different", parent=self.root)
                else:
